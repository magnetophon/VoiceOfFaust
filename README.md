...
It turns any monophonic sound into a synthesizer, preserving the pitch and spectral dynamics of the input.  
...
[These](http://magnetophon.nl/sounds/magnetophon/oldCombo.flac) [are](https://raw.githubusercontent.com/magnetophon/VoiceOfFaust/MasterSlave/Caribean.mp3) some silly demo songs.
...
* Optionally use as a [master-slave](#master-slave) pair  
  ...
  * The outputs of the master can be recorded into a looper or DAW,  
  ...
  This is a workaround for the need for an external pitchtracker, making it impossible to use these as plugins.
...
* 1 = creates a separate oscillator for each output channel,
...
Together with the [output routing], it can also create interesting cancellation effects.
